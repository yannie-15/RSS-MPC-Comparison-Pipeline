...
%
% 输出格式与 main 一致:
%   - paper_reproduction/results/per_algorithm/{算法名}/{算法名}_summary.png
%   - paper_reproduction/results/per_algorithm/{算法名}/seed_0001.png
%   - paper_reproduction/results/csv/{算法名}_results.csv
%   - paper_reproduction/results/paper_reproduction.mat
%
...
% 完全独立脚本: 删除本文件不影响其他功能。
    ...
    for i = 1:num_algorithms
        alg = algorithms{i};
        csv_file = fullfile(csv_dir, sprintf('%s_results.csv', alg));
...
function save_algorithm_csv_paper(comparison, alg_idx, results_dir)
    ...
    csv_file = fullfile(csv_dir, sprintf('%s_results.csv', algorithm));

    try
        writetable(T, csv_file);
        fprintf('[算法 %s] CSV 已保存: %s\n', algorithm, csv_file);
    catch
        csv_tmp = fullfile(csv_dir, sprintf('%s_results_new.csv', algorithm));
