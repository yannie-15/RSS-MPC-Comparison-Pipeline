...
% 在运行任何仿真脚本前执行此文件
%
% 注: 三个算法 submodule (algorithms/RSS_proposed, RSS_sqp, RSS_fmincon)
%     不在此处统一 addpath, 因为它们的 control_RSS.m 和 config.m 同名会冲突。
%     run_one_case.m 会在调用时临时 addpath/rmpath 切换。
%     algorithms/ 目录本身需要 addpath, 用于本地 control_active_set.m。
%
% 用法：
...
function setup_paths()
    ...
    fprintf('  - matlab/\n');
    fprintf('  - algorithms/ (本地 active-set + 3 个 submodule, submodule 由 run_one_case.m 动态切换)\n');
end
